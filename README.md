...
2. **成本是線性估算**:0.6% 來回(關4 掃 0.4%~0.8%),沒有模擬滑價、漲跌停買不到、流動性衝擊。
   小型股實際成本更高 → 又是偏樂觀方向。且成本每期扣滿(假設換手 100%)、基準 EW 零成本
   → 超額的硬門檻 ≈ 0.6%/期(年化 ~7.5%);持股延續性高的策略被略微高估成本(偏嚴方向)。
3. **樣本期 regime 污染**:台股 2021-25 幾乎無正常年(關5 會自動警示),
   在這段驗出的任何結論外推性都有限。
4. **預設用原始價(未還原)**:`TaiwanStockPrice` 不含除權息/分割調整。兩個實測後果
   (2026-07-11 審計,資料 2021-06~2024-12):
   - 掃出 **65 筆單日 |漲跌|>30%** 假跳動(分割/減資,台股漲跌停僅 10%)→ 污染動能類訊號;
   - 除息假跌 → **高殖利率/存股型訊號的報酬被系統性低估**(≈ 選股殖利率超出市場平均的部分)。
   驗這類訊號請切還原價:`export QLAB_PRICE=adj` 後重跑 backfill(資料集自動換
   `TaiwanStockPriceAdj`,檔名 `kline_deep_adj.json`,與 raw 並存;engine 讀同一 env,一鍵切全管線)。
5. **樣本外要有資料才算關**:資料尾 < OOS 切點時,關2 印 🚨 空轉警告(2026-07-11 加)——
   看到警告先 `python backfill/update_data.py` 補資料,別把四關當五關。

> 2026-07-11 真資料審計紀錄:CHEAT(偷看未來)= +20.9%/期、勝率 100%、p=0;
> 純亂數 = −0.51%/期 ≈ −成本(0.6%)。引擎抓得到真 edge、也不冤枉沒 edge 的訊號 —— 數學自洽。
